...
$\Omega_{p1} = 0.7028$, $\Omega_{p2} = 0.8272$ and $\Omega_{s1} = 0.6773$, $\Omega_{s2} = 0.8650$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.762$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1244$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.446\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.557
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.446.
\end{align}
...
we get $N\geq 4$ and $0.342 \leq \epsilon \leq 0.61$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/SujanRada/EE1205/tree/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.342  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/SujanRada/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/RadaSujan/EE1205/blob/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
     ...
     &= \frac{0.3125}{s_{L}^4 + 1.1068s_{L}^3 +1.6124s_{L}^2 + 0.9139s_{L} +0.3365}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{2.53\times 10^{-4}s^4}{s^8 + 0.1376s^7 + 2.3475s^6 + 0.2415s^5 + 2.0519s^4 + 0.1402s^3 + 0.7914s^2 + 0.0269s + 0.1136} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{6.85 - 14.18z^{-1} + 37.07z^{-2} - 44.35z^{-3} + 58.87z^{-4} - 42.46z^{-5} + 33.98z^{-6} - 12.44z^{-7} + 5.75z^{-8}}
\end{align}

where $G=2.53\times 10^{-4}$    
\begin{figure}[H]
\centering
...
\begin{align}
    \omega_{l} &= \frac{\omega_{p2}-\omega_{p1}}{2}\\
                &= 0.025\pi
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.415n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.415n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF3}
\end{align}
